        ...
        if clsname == 'Arrow':
            ...
            for single_gen_attrs, single_gt_attrs, gt_indice, single_gt_covered in zip(cls_gen_attrs, cls_gt_attrs, tp_gt, gt_covered):
                if len(single_gen_attrs) != 0 and len(np.array(single_gen_attrs)[gt_indice][single_gt_covered]) != 0:
                    all_pred_attrs.append(np.array(single_gen_attrs)[:,0][:,np.newaxis][gt_indice][single_gt_covered])
                    all_gt_attrs.append(np.array(single_gt_attrs)[:,0][:,np.newaxis][single_gt_covered])
            if len(all_pred_attrs):
                ...
                for single_class_pred_attrs, single_class_gt_attrs in zip(all_pred_attrs, all_gt_attrs):
                    ...
                    attrs_metric[clsname] = {'precision':single_attrs_precision, 'recall':single_attrs_recall, 'f1_score':single_attrs_f1}
            else:
                for attr_class in list(attrs_dict.keys()):
                    attrs_metric[attr_class] = {'precision':0.0, 'recall':0.0, 'f1_score':0.0}
        if clsname == 'ExclusiveLaneSign':
            # import pdb;pdb.set_trace()
            for single_gen_attrs, single_gt_attrs, gt_indice, single_gt_covered in zip(cls_gen_attrs, cls_gt_attrs, tp_gt, gt_covered):
                if len(single_gen_attrs) != 0 and len(np.array(single_gen_attrs)[gt_indice][single_gt_covered]) != 0:
            ...
            if len(all_pred_attrs):
                ...
                attrs_metric[clsname] = {'precision':0.0, 'recall':0.0, 'f1_score':0.0}
        # calculate AP
...
def print_map_summary(mean_ap,
    ...
    for i in range(num_scales):
        ...
        for j in range(num_classes):
            ...
            if label_names[j] in attrs_dict.keys():
                attrs_metric = results[j]['attrs_metric']
                attrs_row_data = [
                    label_names[j], attrs_metric[label_names[j]]['precision'], attrs_metric[label_names[j]]['recall'], attrs_metric[label_names[j]]['f1_score'] 
                ]
