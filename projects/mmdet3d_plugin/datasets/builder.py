...
def build_dataloader_icurb(dataset,
                     samples_per_gpu,
                     workers_per_gpu,
                     num_gpus=1,
                     dist=True,
...
def iCurb_collate(batch):
    data = {}
    data['seq'] = [x[0] for x in batch]
    data['shift_pts'] = [x[1] for x in batch]
    data['img'] = torch.stack([x[2] for x in batch])
    data['gt_labels'] = [x[3] for x in batch]
    data['img_metas'] = [x[4] for x in batch]
    return data
