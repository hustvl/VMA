...
class VectorizedLocalMap(object):
    ...
    def gen_vectorized_samples(self, instances):
        ...
        for instance in instances:
            ...
            vectors.append((LineString(np.array(instance_data)), self.vec_classes.index(instance_class))) 
        gt_labels = []
        gt_instances = []
        gt_attrs = []
        ...
        gt_instances = InstanceLines(self.vec_classes, gt_instances, gt_labels, self.sample_dist,
...
class SD_Driving_Freespace_Dataset(Dataset):
    r'''
    ...
    def vectormap_pipeline(self, example, instances):
        ...
        anns_results = vectormap.gen_vectorized_samples(instances)
    ...
    def _evaluate_single(self,
        ...
        """
        from projects.mmdet3d_plugin.datasets.map_utils.sd_freespace_evaluate import eval_map, format_res_gt_by_classes
        ...
        for metric in metrics:
            ...
            for i, name in enumerate(self.MAPCLASSES):
                ...
                detail['SD_Freespace_Map_{}/{}_AP'.format(metric,name)] =  cls_aps.mean(0)[i]
            print('map: {}'.format(cls_aps.mean(0).mean()))
            detail['SD_Freespace_Map_{}/mAP'.format(metric)] = cls_aps.mean(0).mean()

            for i, name in enumerate(self.MAPCLASSES):
                for j, thr in enumerate(thresholds):
                    if metric == 'chamfer':
                        detail['SD_Freespace_Map_{}/{}_AP_thr_{}'.format(metric,name,thr)]=cls_aps[j][i]
                    elif metric == 'iou':
    ...
    def arrange_results_and_annotations(self, results, annotations):
        ...
        for pred_result, gt_annotation in zip(results.values(), annotations.values()):
            ...
            for instance in pred_instances:
                ...
                single_image_pred_results.append({'pts':instance_data,'type':instance['class'], 'confidence_level':instance['confidence_level']})
            gt_instances = gt_annotation['instances']
            for instance in gt_instances:
                single_image_gt_annotations.append({'pts':instance['data'],'type':self.MAPCLASSES.index(instance['class'])})
    ...
    def show_result(self,
        ...
        for pred_instance in pred_instances:
            ...
            sub_pred_points = [tuple([round(x[0]), round(x[1])]) for x in sub_pred_points]
