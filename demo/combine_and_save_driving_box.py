...
def combine_results_and_save_driving_box(config, img, results, out_dir=None, left_top=None, visualize_flag=False):
    print('Start to aggregate driving box instances')
    image_name = img.split('/')[-1]

    box_class = config['map_classes']
    arrow_attrs = config['arrow_attrs']
    exclusivelanesign_attrs = config['exclusivelanesign_attrs']
    ...
    for i in range(len(results)):
        ...
        for pred_instance in pred_instances:
            ...
            big_results.append({'type':box_class[pred_instance['class']], 'seq':instance_data, 'confidence_level':pred_instance['confidence_level'],'attrs':pred_instance['attrs_labels']})
    cls_gens = format_res_gt_by_classes_box_with_attr(big_results,
                                            cls_names=box_class,
                                            num_pred_pts_per_instance=5,
                                            num_attrs=2)
    for i, clsname in enumerate(box_class):
        ...
        cls_aggr_gen = cls_aggr_gen[:,:-1]
        if clsname == 'Crosswalk' or clsname == 'NoParkingLine':
            ...
            for single_cls_aggr_gen in cls_aggr_gen:
                data = np.array(single_cls_aggr_gen).reshape(-1,2).astype(np.int32)
                rect = cv2.minAreaRect(data)
                data = cv2.boxPoints(rect).tolist()
                aggr_pred_instances.append({'type':clsname, 'seq':data, 'attrs':'none'})
        elif clsname == 'Arrow':
            for single_cls_aggr_gen in cls_aggr_gen:
                data = np.array(single_cls_aggr_gen[:-2]).reshape(-1,2).tolist()[:-1]
                pred_arrow_attrs = arrow_attrs[int(single_cls_aggr_gen[-1])]
                attrs_list = pred_arrow_attrs.split('-')
                aggr_pred_instances.append({'type':clsname, 'seq':data, 'attrs':attrs_list})
        elif clsname == 'ExclusiveLaneSign':
            for single_cls_aggr_gen in cls_aggr_gen:
                data = np.array(single_cls_aggr_gen[:-2]).reshape(-1,2).astype(np.int32)
                rect = cv2.minAreaRect(data)
                data = cv2.boxPoints(rect).tolist()
                aggr_pred_instances.append({'type':clsname, 'seq':data, 'attrs':exclusivelanesign_attrs[int(single_cls_aggr_gen[-2])]})
        else:
            for single_cls_aggr_gen in cls_aggr_gen:
                data = np.array(single_cls_aggr_gen[:-2]).reshape(-1,2).astype(np.int32)
                rect = cv2.minAreaRect(data)
                data = cv2.boxPoints(rect).tolist()
                aggr_pred_instances.append({'type':clsname, 'seq':data, 'attrs':'none'})
    print('driving box aggregated!')
    vector_outdir = os.path.join(out_dir, 'vector_out')
    if not os.path.exists(vector_outdir):
        os.makedirs(vector_outdir)
        
    json_path = os.path.join(vector_outdir, image_name.replace('.jpg', '_driving_box.json'))
    ...
    if visualize_flag:
        ...
        visualize_single_image_box(img, aggr_pred_instances, visualize_aggregated_out_dir)

def visualize_single_image_box(image_path, pred_instances, out_dir):
    # import pdb;pdb.set_trace()
    image = cv2.imread(image_path)
    drew_image_path = '/'.join([out_dir, image_path.split('/')[-1].replace('.jpg', '_driving_box.jpg')])
    for pred_instance in pred_instances:
...
def gen_distinct_instances(gen_lines,
    ...
    gen_scores = gen_lines[:,-(num_attrs+1)] # n
    
    # distance matrix: n x m
    matrix = custom_polyline_score_distinct(
            gen_lines[:,:-(num_attrs+1)].reshape(num_gens,-1,2), 
            gt_lines[:,:-(num_attrs+1)].reshape(num_gts,-1,2),
            linewidth=line_width,
            metric=metric)
...
def format_res_gt_by_classes_box_with_attr(gen_results,
                                 cls_names=None,
                                 num_pred_pts_per_instance=5,
                                 num_attrs=0):
    ...
    for i, clsname in enumerate(cls_names):
        ...
        for gen_result in gen_results:
            if gen_result['type'] == clsname:
                ...
                cls_attrs.append(gen_result['attrs'])
        num_res = len(cls_gen)
        if num_res > 0:
            cls_gen = np.stack(cls_gen).reshape(num_res,-1)
            cls_score = np.array(cls_score)[:,np.newaxis]
            cls_attrs = np.stack(cls_attrs)
            cls_gen = np.concatenate([cls_gen, cls_score, cls_attrs],axis=-1)
        else:
