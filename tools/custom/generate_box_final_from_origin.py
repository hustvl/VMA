...
def generate_box_final_from_original(all_gt_6k_json_file_path, data_root, out_dir, visualize_flag):
    ...
    # results = [process_box_one_single_image(image_ann, cropped_save_dir) for image_ann in tqdm(json_list)]
    with multiprocessing.Pool(processes = 64) as pool:
        partial_func = partial(process_box_one_single_image, data_root=data_root, save_dir=cropped_save_dir)
    ...
    result_1k_gt_dict_json_path = os.path.join(out_dir, 'sd_data_box_dict.json')
    ...
    if visualize_flag:
        ...
        for small_image_annotations in tqdm(result_dict.values(), desc='visualize 1k gt'):
            for small_image_annotation in small_image_annotations:
                visualize_small_image_box(small_image_annotation, cropped_save_dir, visualize_dir)
    print('visualization completed')

def visualize_small_image_box(image_ann, data_root, out_dir):
...
def process_box_one_single_image(image_ann, data_root, save_dir):
    ...
    instances_list = extract_box_useful_data(image_ann)
    trajectory_data_list = extract_trajectory_data(trajectory_json_path)
    small_image_data_list = extract_box_1k_small_image_instances(trajectory_data_list, instances_list, image_name, image_path, save_dir)
...
def extract_box_useful_data(img_ann):
    wanted_box_class = ['Crosswalk', 'SpeedBump', 'NoParkingLine', 'Arrow', 'Diamond_marking', 'ExclusiveLaneSign', 'InvertedTriangleMarking', 'StopLine']
    all_attrs_set = [{'Mergeright'}, {'Mergeleft'}, {'Turnright'}, {'Turnleft'}, {'Straight'}, {'Turnoff'}, {'Straight', 'Turnleft'}, {'Turnright', 'Straight'}, {'Turnright', 'Turnleft'}, {'Other'}, {'Turnoff', 'Turnleft'}, {'Turnright', 'Straight', 'Turnleft'}, {'Turnoff', 'Straight'}, {'Turnright', 'Turnoff'}, {'Turnright', 'Other', 'Straight', 'Turnleft'}, {'Other', 'Turnleft'}, {'No_Turnoff'}, {'Straight', 'Mergeleft'}, {'No_Straight', 'Straight'}]
    merged_attrs = [  'Mergeright',   'Mergeleft',   'Turnright',   'Turnleft',   'Straight',   'Turnoff',   'Straight-Turnleft',      'Straight-Turnright',      'Turnright-Turnleft',      'Other',   'Turnoff-Turnleft',      'Turnright-Straight-Turnleft', 'Turnoff-Straight',              'Turnright-Turnoff',      'Turnright-Other-Straight-Turnleft',            'Other-Turnleft',      'No_Turnoff',   'Straight-Mergeleft',       'No_Straight-Straight']
    instances_list = []
    if 'parsing' not in img_ann.keys():
        for key, value in img_ann.items():
            if key in wanted_box_class:
                for instance in value:
                    new_instance_ann = {}
                    new_instance_ann['class'] = key
                    data = instance['data']
                    if isinstance(data[0], list):
                        new_instance_ann['data'] = data[0] + [data[0][0]]
                    else:
                        new_instance_ann['data'] = [[data[0], data[1]], [data[0], data[3]], [data[2], data[3]], [data[2], data[1]], [data[0], data[1]]]
                    attrs = instance['attrs'].pop('Attribution')
                    if set(attrs) in all_attrs_set:
                        new_instance_ann['attrs'] = merged_attrs[all_attrs_set.index(set(attrs))]
                    else:
                        new_instance_ann['attrs'] = attrs
                    instances_list.append(new_instance_ann.copy())
            # else: 
            #     print(key)
    return instances_list

def extract_box_1k_small_image_instances(trajectory_data_list, instances_list, image_name, image_path, save_dir):
    ...
    for trajectory_data in trajectory_data_list:
        ...
        for sampled_point in sampled_points:
            ...
            for instance in instances_list:
                ...
                if instance_cls == 'Arrow':
                    if isinstance(line, Polygon):
                        new_instance_length = len(list(line.exterior.coords))
                        if new_instance_length == 5:
                            new_instance_data = np.array(list(line.exterior.coords))
                            if ((new_instance_data == np.array(new_instance_pts)).all()):
                            
                                new_coords = [[x[0]-bbox_minx, x[1]-bbox_miny] for x in new_instance_data]
                                small_instances_data.append({'class':instance_cls, 'data':new_coords, 'attrs':instance_attr})
                            else:
                                new_instance_data = np.flip(new_instance_data[:-1], axis=0)
                                new_instance_data = np.concatenate((new_instance_data,np.expand_dims(new_instance_data[0],axis=0)),axis=0)
                                if ((new_instance_data == np.array(new_instance_pts)).all()):
                                    new_coords = [[x[0]-bbox_minx, x[1]-bbox_miny] for x in new_instance_data]
                                    small_instances_data.append({'class':instance_cls, 'data':new_coords, 'attrs':instance_attr})                                           
                else:
                    if isinstance(line, Polygon): 
                        if len(list(line.exterior.coords))==5:
...
if __name__ == '__main__':
    ...
    generate_box_final_from_original(args.all_gt_6k_json_file_path, args.data_root, args.out_dir, args.visualize)